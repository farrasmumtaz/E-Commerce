import React, { useState, useEffect, useRef } from "react";

export default function Profil({
  pindahHalaman,
  pesanan,
  pengguna,
  aturPengguna,
  aturKeranjang,
  aturPesanan,
}) {
  const [dataFormulir, aturDataFormulir] = useState({
    nama: pengguna.nama || "",
    email: pengguna.email || "",
    telepon: pengguna.telepon || "",
    alamat: pengguna.alamat || "",
  });
  const [tersimpan, aturTersimpan] = useState(false);
  const emailSebelumnya = useRef(pengguna.email);

  useEffect(() => {
  if (emailSebelumnya.current !== pengguna.email) {
    emailSebelumnya.current = pengguna.email;
    ({
      nama: pengguna.nama || "",
      email: pengguna.email || "",
      telepon: pengguna.telepon || "",
      alamat: pengguna.alamat || "",
    });
  }
}, [pengguna]);

  const tanganiSimpan = (e) => {
    e.preventDefault();
    aturPengguna(dataFormulir);
    aturTersimpan(true);
    setTimeout(() => aturTersimpan(false), 3000);
  };

  const tanganiLogout = () => {
    localStorage.removeItem("token");
    aturKeranjang([]);
    aturPesanan([]);
    aturPengguna({
      nama: "",
      email: "",
      telepon: "",
      alamat: "",
      role: "",
    });
    pindahHalaman("login");
  };

  return (
    <div className="wadah">
      <h2>Dasbor Profil</h2>
      <div className="tata-letak-baris">
        <div className="kolom-1 kotak-ringkasan">
          <div className="ikon-profil">👤</div>
          <h3 className="teks-tengah">{pengguna.nama}</h3>
          <p className="email-profil">{pengguna.email}</p>
          <hr className="garis-pembatas" />
          <h3>Informasi Pribadi</h3>
          {tersimpan && (
            <p className="pesan-sukses">✓ Profil berhasil disimpan!</p>
          )}
          <form onSubmit={tanganiSimpan} className="jarak-atas-sedikit">
            <div className="grup-formulir">
              <label>NAMA</label>
              <input
                type="text"
                value={dataFormulir.nama}
                onChange={(e) =>
                  aturDataFormulir({ ...dataFormulir, nama: e.target.value })
                }
                required
              />
            </div>
            <div className="grup-formulir">
              <label>NOMOR TELEPON</label>
              <input
                type="text"
                value={dataFormulir.telepon}
                onChange={(e) =>
                  aturDataFormulir({ ...dataFormulir, telepon: e.target.value })
                }
              />
            </div>
            <div className="grup-formulir">
              <label>ALAMAT</label>
              <input
                type="text"
                value={dataFormulir.alamat}
                onChange={(e) =>
                  aturDataFormulir({ ...dataFormulir, alamat: e.target.value })
                }
              />
            </div>
            <button type="submit" className="tombol-utama">
              Simpan Perubahan
            </button>
            <button
              type="button"
              className="tombol-garis tombol-bahaya"
              onClick={tanganiLogout}
            >
              Keluar (Logout)
            </button>
          </form>
        </div>

        <div className="kolom-1-5">
          <h3>Riwayat Pesanan ({pesanan.length})</h3>
          {pesanan.length === 0 ? (
            <p className="daftar-pesanan-kosong">Anda belum memiliki pesanan.</p>
          ) : (
            <div className="jarak-atas-sedikit">
              {pesanan.map((order) => (
                <div key={order.id} className="kartu-pesanan">
                  <div className="header-pesanan">
                    <span className="id-pesanan">{order.id}</span>
                    <span className="status-pesanan">{order.status}</span>
                  </div>
                  <p className="tanggal-pesanan">Dipesan pada: {order.tanggal}</p>
                  {order.items.map((item, idx) => (
                    <div key={idx} className="baris-item-pesanan">
                      <span>{item.nama} ×{item.kuantitas}</span>
                      <span>Rp {(item.harga * item.kuantitas).toLocaleString()}</span>
                    </div>
                  ))}
                  <hr className="garis-putus" />
                  <div className="baris-total-pesanan">
                    <span>Total</span>
                    <span>Rp {order.total.toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
          )}
        </div>
      </div>
    </div>
  );
}